<template>
	<div>
		...
		<!-- Header -->
		<div
			class="bg-surface border border-border rounded-xl p-4 md:p-6 mb-4 md:mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4"
		>
			<div>
				...
				<p class="text-sm text-text-secondary mb-3">
					Daftar responden yang mengerjakan {{ currentLikert?.name }}
				</p>
				<span
		...
		<div class="flex flex-wrap justify-end gap-2 mb-2">
			<button
				...
				class="inline-flex items-center justify-center gap-2 px-4 py-2.5 md:py-2 text-sm font-medium rounded-sm border border-black-secondary text-primary transition-colors whitespace-nowrap h-10 disabled:opacity-50 cursor-pointer"
			>
				<font-awesome-icon
			...
			<div class="overflow-x-auto">
				<table class="app-table w-full text-left border-collapse">
					<thead>
						<tr>
							<th
								class="px-4 md:px-5 py-3 text-xs font-medium uppercase tracking-wider border-0"
							>
								Nama
							</th>
							<th
							...
							<th
								class="px-4 md:px-5 py-3 text-xs font-medium uppercase tracking-wider w-12 border-0"
							></th>
						</tr>
					</thead>
					<tbody class="divide-y divide-border">
						<tr
							...
							@click="
								router.push({
									...
									params: { slug: likertSlug, submissionSlug: s.slug },
								})
							"
							class="cursor-pointer"
						>
							<td class="px-4 md:px-5 py-3 text-sm font-medium">
								{{ s.name }}
							</td>
							<td class="px-4 md:px-5 py-3 text-sm">{{ s.school }}</td>
							<td class="px-4 md:px-5 py-3 text-sm">{{ s.class }}</td>
							...
							<td class="px-4 md:px-5 py-3 text-sm">
								{{ computeTotalScore(s.submission) || "-" }}
							</td>
							<td class="px-4 md:px-5 py-3 text-sm whitespace-nowrap">
						...
						<tr v-if="submissions.length === 0">
							<td
								colspan="8"
								class="px-4 md:px-5 py-6 text-sm text-center text-text-muted"
							>
		...
		>
			...
			>
				...
				<p class="text-sm text-text-secondary leading-relaxed mb-6">
					File berisi rekap {{ submissions.length }} responden akan diunduh
					dalam format .xlsx.
				</p>
