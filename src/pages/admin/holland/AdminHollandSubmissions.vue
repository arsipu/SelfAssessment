		...
		>
			<div>
				...
				<p class="text-sm text-text-secondary mb-3">
					Daftar responden yang mengerjakan Holland RIASEC
				</p>
				<span
				...
				<table class="app-table w-full text-left border-collapse">
					<thead>
						<tr>
							...
							<th
								class="px-4 md:px-5 py-3 text-xs font-medium uppercase tracking-wider w-12"
							></th>
						</tr>
					</thead>
					<tbody class="divide-y divide-border">
						<tr
							...
							@click="
								router.push({
									...
									params: { slug: hollandSlug, submissionSlug: s.slug },
								})
							"
						...
						>
							...
							<td class="px-4 md:px-5 py-3 text-sm font-semibold tracking-wide">
								{{ getTopCode(s) }}
							</td>
...
function getTopCode(submission) {
	...
	const breakdown = computeScoreBreakdownFromAnswers(
		submission.answers,
		riasecIds.value,
	);
	return computeTopCode(breakdown, 3) || "-";
}
