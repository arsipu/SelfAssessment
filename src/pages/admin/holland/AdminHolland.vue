<template>
	<div>
		...
		<div class="bg-surface mb-2">
			<div>
				...
				<p class="text-sm text-text-secondary max-w-3xl mb-3">
					Kelola daftar instrumen Holland. Klik instrumen untuk mengelola
				...
				<table class="app-table w-full text-left border-collapse">
					...
					<tbody class="divide-y divide-black/30">
									...
									<Teleport to="body">
										...
										>
											<button
												...
												:class="{
													'bg-surface-muted font-medium': h.status === s.value,
												}"
											>
												<span
													class="w-1.5 h-1.5 rounded-full shrink-0"
						...
						<tr v-if="hollands.length === 0">
							<td
								colspan="5"
								class="px-4 md:px-5 py-8 text-sm text-center text-text-muted"
							>
				...
				<div class="px-6 py-4 flex justify-between items-center shrink-0">
					<div>
						...
						<p class="text-xs text-text-muted mt-0.5">
							{{
								isEditing
									? "Ubah nama, deskripsi, atau petunjuk instrumen."
									: "Buat instrumen Holland RIASEC baru."
							}}
						</p>
					</div>
					<button
				...
				<div class="p-6 space-y-4 overflow-y-auto">
					<div>
						<label class="block text-sm font-medium text-black mb-1"
							>Nama Instrumen <span class="text-danger">*</span></label
						>
						<input
							v-model="form.name"
							type="text"
					...
					<div>
						...
						<p class="text-xs text-text-muted mt-1">
							Ditampilkan di halaman instrumen sebelum tes dimulai.
						</p>
					</div>
				</div>
		...
		>
			Apakah Anda yakin ingin menghapus instrumen ini? Semua data terkait —
			termasuk kategori RIASEC, pertanyaan, kolom, dan submission —
			<strong class="text-text-primary">akan terhapus permanen</strong>.
...
const statusLabel = (status) => {
	return (
		statusOptions.find((s) => s.value === status)?.label || statusText(status)
	);
};
