					...
					<div class="p-5 space-y-4">
						...
						<div>
							...
							<div v-else class="space-y-2">
								...
								>
									<div
										class="w-8 h-8 rounded-full bg-success-soft flex items-center justify-center text-xs font-bold text-success shrink-0"
									>
										{{ initialsOf(r.name) }}
									</div>
					...
					<div class="p-5 space-y-4">
						...
						<div>
							...
							<div v-else class="space-y-2">
								...
								>
									<div
										class="w-8 h-8 rounded-full bg-primary-soft flex items-center justify-center text-xs font-bold text-primary shrink-0"
									>
										{{ initialsOf(r.name) }}
									</div>
									<div class="flex-1 min-w-0">
	...
	return [
		{
			...
			value: total,
			sub: "Seluruh instrumen",
			positive: true,
		},
		{
			label: "Asesmen selesai",
			value: completed,
			sub: `${completedPct}% dari total`,
			positive: true,
		},
		{
...
const globalStats = computed(() => {
	...
	const totalInstruments =
		likertProgress.value.length + hollandProgress.value.length;
...
const likertRecentRespondents = computed(() =>
	buildRecentRespondents(likertSubmissions.value),
);
const hollandRecentRespondents = computed(() =>
	buildRecentRespondents(hollandSubmissions.value),
);
...
onMounted(async () => {
	...
	try {
		...
		const activedLikers = (likertStore.likerts || []).filter(
			(l) => l.status === ACTIVE,
		);
		const activedHollands = (hollandStore.hollands || []).filter(
			(h) => h.status === ACTIVE,
		);
		...
		const [likertResults, hollandResults] = await Promise.all([
			Promise.all(
				activedLikers.map(async (l) => {
					...
					return {
						instrument: l,
						submissions: [...likertSubmissionsStore.submissions],
					};
				}),
			),
			Promise.all(
				activedHollands.map(async (h) => {
					...
					return {
						instrument: h,
						submissions: [...hollandSubmissionsStore.submissions],
					};
				}),
			),
		]);
