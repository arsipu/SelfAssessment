<template>
	<div class="min-h-screen">
			...
			>
				<div class="flex flex-col md:flex-row items-center gap-10 md:gap-16">
					...
					<div class="flex-1 text-center md:text-left">
						<!-- <span
							class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-text-primary bg-white text-xs font-semibold tracking-wide mb-5"
						>
						...
						<p
							class="text-sm md:text-base text-text-secondary leading-relaxed mb-8 max-w-lg mx-auto md:mx-0"
						>
							Kumpulan instrumen asesmen diri psikologi terstandar — diisi
							online, hasil langsung terlihat begitu selesai. Gratis, tanpa
							daftar.
						</p>
						...
						>
							<a
								href="#instrumen"
								class="inline-flex items-center gap-2 px-6 py-3.5 btn-primary text-sm font-semibold rounded-xl hover:bg-primary-hover transition active:scale-[0.98] shadow-sm"
							>
								Mulai asesmen
								<font-awesome-icon
		...
		>
			...
			>
				<font-awesome-icon
					...
					class="w-7 h-7 animate-spin mb-3"
				/>
			...
			<div v-else class="grid grid-cols-1 sm:grid-cols-2 gap-5 md:gap-6">
				<router-link
					...
					:key="item.key"
					:to="item.to"
					class="card group relative bg-surface border border-border rounded-2xl p-6 hover:border-primary hover:shadow-md transition-all duration-300 active:scale-[0.99]"
				>
					<div class="flex items-start justify-between mb-4">
						<div
							class="w-12 h-12 rounded-xl flex items-center justify-center group-hover:bg-primary transition-colors duration-300"
						>
							<font-awesome-icon
								:icon="
									item.type === 'holland'
					...
					<h3
						class="text-base font-bold text-black mb-2 group-hover:text-primary transition-colors"
					>
						{{ item.name }}
					</h3>
					<p
						class="text-sm text-black-secondary leading-relaxed mb-6 min-h-[3em]"
					>
		...
		<section class="py-8 md:py-8">
			<div class="max-w-full mx-16 px-4 md:px-6">
				<div class="text-center mb-12">
					<p
						class="text-xs font-semibold tracking-wide uppercase text-primary mb-2"
					>
						Cara kerja
					</p>
					<h2 class="text-xl md:text-2xl font-bold text-text-primary">
				...
				<div class="grid grid-cols-1 sm:grid-cols-3 gap-10 md:gap-12 relative">
					...
					<div
						class="hidden sm:block absolute top-7 left-[15%] right-[15%] h-px bg-border -z-10"
					></div>
					...
					>
						...
						<p
							class="text-sm text-text-secondary leading-relaxed max-w-[240px]"
						>
							{{ step.desc }}
						</p>
					</div>
