<template>
	<div class="max-w-2xl mx-auto px-4 py-6 pb-20 sm:py-10">
		...
		<div class="bg-surface card rounded-2xl overflow-hidden">
			<div class="mb-5 sm:mb-6 card-title p-6">
			...
			<form @submit.prevent="goToKuesioner" class="space-y-4 sm:space-y-5 p-6">
				<div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
					...
					<div class="flex flex-col gap-1">
						<label class="text-sm font-semibold text-black"
							>Sekolah/Universitas <span class="text-danger">*</span></label
						>
						<input
							v-model="responden.school"
							type="text"
					...
					<div class="flex flex-col gap-1">
						<label class="text-sm font-semibold text-black">Pekerjaan</label>
						<input
							v-model="responden.occupation"
							type="text"
					...
					<div class="flex flex-col gap-1">
						<label class="text-sm font-semibold text-black"
							>Tanggal Tes <span class="text-danger">*</span></label
						>
						<input
							v-model="responden.testDate"
							type="date"
							required
							class="px-3 py-2.5 border border-border-primary rounded-lg text-sm bg-surface focus:outline-none focus:border-primary focus:bg-surface transition"
						/>
					...
					<div class="sm:col-span-2 flex flex-col gap-1">
						<label class="text-sm font-semibold text-black">Tujuan Tes</label>
						<input
							v-model="responden.testPurpose"
							type="text"
							placeholder="Contoh: Bimbingan karier"
							class="px-3 py-2.5 border border-border-primary rounded-lg text-sm bg-surface focus:outline-none focus:border-primary focus:bg-surface transition"
						/>
					</div>
				</div>
				...
				<button
					...
					class="w-full mt-2 py-3 btn-primary text-sm font-semibold rounded-xl transition active:scale-[0.98] disabled:opacity-50 disabled:pointer-events-none cursor-pointer"
				>
					{{ submitButtonLabel }}
				</button>
			</form>
...
async function prepareQuestions() {
	...
	try {
		...
		if (allQuestions.value.length === 0) {
			loadError.value =
				"Belum ada pertanyaan yang tersedia untuk instrumen ini.";
		}
	} catch (error) {
