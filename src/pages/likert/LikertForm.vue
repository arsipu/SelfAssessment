	...
	if (currentLikert.value === null) {
		router.push({
			...
			query: {
				...
				message:
					"Kuesioner yang kamu cari mungkin sudah dihapus atau link tidak valid.",
			},
		});
		return;
	}
...
async function goToKuesioner() {
	...
	try {
		await likertSessionStore.startSession(likertId.value, {
			...responden.value,
		});
		router.push({ name: "likert-questions", params: { slug: likertSlug } });
	} catch (error) {
