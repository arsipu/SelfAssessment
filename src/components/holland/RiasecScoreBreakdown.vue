<template>
	<div
		:class="
			variant === 'table'
				? ''
				: 'bg-surface border border-border rounded-2xl p-5 md:p-6 shadow-sm'
		"
	>
		<p v-if="title" class="text-xs font-medium text-black mb-3">
			{{ title }}
		</p>
		...
		<table v-if="variant === 'table'" class="w-full text-sm border-collapse">
			<!-- <thead>
				<tr class="border-b border-border">
					<th
						class="text-left py-1.5 px-1 text-[11px] font-normal text-text-muted"
					>
						Kategori
					</th>
				</tr>
			...
			<tbody>
				...
				>
					<td class="w-36 py-2 px-1 font-medium text-black">
						{{ getLabel(row.code) }}
						<span class="text-black-secondary font-normal"
							>({{ row.code }})</span
						>
					</td>
					...
					<td
						class="py-2 px-1 text-right text-black-secondary whitespace-nowrap w-16"
					>
						{{ row.percentage }}%
					</td>
				</tr>
		...
		<p
			v-if="variant !== 'table'"
			class="text-[11px] text-text-muted mt-4 leading-relaxed"
		>
			Persentase dihitung dari jumlah pernyataan yang dipilih dibagi total
			pernyataan pada kategori tersebut, karena jumlah pernyataan tiap kategori
			tidak sama.
		</p>
